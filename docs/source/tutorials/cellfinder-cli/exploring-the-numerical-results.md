...
In the `test_brain/output/analysis` directory is a `summary.csv` file which you can open in Microsoft Excel 
(or similar) to view a summary of the results. This file lists, for each brain area, the number of cells detected, 
the volume of the brain area, and the density of cells (in cells per mm<sup>3</sup>). This is the file you'll most 
likely use for statistical analysis. It will look something like this (but with an entry for each brain area):	
...
These data allow you to compare data from multiple samples, but to visualise data from different samples in the same 
coordinate space, take a look at [Visualising your data in brainrender](visualising-your-data-in-brainrender).
